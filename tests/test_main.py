def login(flask_client, username, password, remember="", next=""):
    return flask_client.post(
        "/login",
        data=dict(wyr_username=username, wyr_password=password, remember=remember, next=next),
...
def test_read1(flask_client, user4):
    ...
    assert b"First user doc" in response.data and b"Second user doc" not in response.data
...
def test_to_read1(flask_client, user4):
    ...
    assert b"Second user doc" in response.data and b"First user doc" not in response.data
...
def test_tags1(flask_client, user4):
    ...
    assert b"tag0" in response.data and b"JUMP TO" in response.data
...
def test_docs_by_tag1(flask_client, user4):
    ...
    assert (
        ...
        and b"Second user doc" not in response.data
    )
...
def test_docs_by_tag2(flask_client, user4):
    ...
    assert (
        b"First user doc" in response.data
        and b"Third user doc" not in response.data
        and b"Second user doc" not in response.data
    )
...
def test_authors1(flask_client, user4):
    ...
    assert b"Smith, Joe" in response.data and b"JUMP TO" in response.data
...
def test_docs_by_author1(flask_client, user4):
    ...
    assert (
        ...
        and b"Third user doc" in response.data
        and b"Second user doc" not in response.data
        and b"Fourth user doc" not in response.data
    )
...
def test_bunch4(flask_client, user6):
    ...
    assert (
        b"First user doc" in response.data
        and b"Second user doc" in response.data
        and b"Third user doc" in response.data
        and b"Fourth user doc" in response.data
    )
...
def test_bunches3(flask_client, user4):
    ...
    response = flask_client.get("/bunches", follow_redirects=True)
    assert (
        b"Create New Bunch" in response.data
        and b"You do not yet have any tags to sort into bunches." not in response.data
    )
...
def test_bunches5(flask_client, user6):
    ...
    response = flask_client.post(
        "/bunches", data=dict(selector="or", bunch_tags=[]), follow_redirects=True
    )
...
def test_bunches6(flask_client, user6):
    ...
    response = flask_client.post(
        "/bunches", data=dict(selector="and", bunch_tags=[3, 5]), follow_redirects=True
    )
    assert (
        b"First user doc" not in response.data
        and b"Second user doc" in response.data
...
def test_bunches7(flask_client, user6):
    """bunches() returns proper docs given chosen selector and tags."""
    response = flask_client.post(
        "/bunches", data=dict(selector="or", bunch_tags=[2, 4]), follow_redirects=True
    )
...
def test_bunches8(flask_client, user6):
    ...
    response = flask_client.post(
        "/bunches", data=dict(selector="and", bunch_tags=[2, 3]), follow_redirects=True
    )
...
def test_bunch_save1(flask_client, user6):

    response = flask_client.post(
        "/bunch/save",
        data=dict(selector="and", bunch_name="bunch5", bunch_tag_ids="3,5"),
        follow_redirects=True,
    )
