...
def user4(flask_client, four_items):
    ...
    flask_client.post(
        "/login",
        data=dict(wyr_username="tester4", wyr_password="testing4", remember="", next=""),
...
def user5(flask_client, four_items):
    ...
    flask_client.post(
        "/login",
        data=dict(wyr_username="tester5", wyr_password="testing5", remember="", next=""),
        follow_redirects=True,
    )
...
def user6(flask_client, five_items):
    ...
    flask_client.post(
        "/login",
        data=dict(wyr_username="tester6", wyr_password="testing6", remember="", next=""),
        follow_redirects=True,
    )
...
def user7(flask_client):
    ...
    flask_client.post(
        "/login",
        data=dict(wyr_username="tester7", wyr_password="testing7", remember="", next=""),
        follow_redirects=True,
    )
...
def developer1(flask_client):
    ...
    flask_client.post(
        "/login",
        data=dict(wyr_username="developer1", wyr_password="developer123", remember="", next=""),
        follow_redirects=True,
    )
    ...
    items.append(
        {
            ...
            "notes": "This is also a note.",
            "read": 0,
            "native_doc_id": "121adb333",
        }
    )  # not used in some of the fixtures, but ok
    items.append(
        {
...
def three_items_tags_only():
    items = []
    items.append({"title": "Test", "tags": ["tag0", "tag1"]})
    items.append({"title": "Test", "tags": ["tag2", "tag3", "tag4"]})
