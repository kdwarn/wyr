...
def get_user_authors(user):
    ...
    authors = (
        Authors.query.join(document_authors)
        .join(Documents)
        .filter(Documents.user_id == user.id)
...
def add_or_update_authors(user, submitted_authors, doc):
    ...
    if doc.authors:
        for author in [
            {"last_name": author.last_name, "first_name": author.first_name}
    ...
    for submitted_author in submitted_authors:
        ...
        if submitted_author not in [
            {"last_name": author.last_name, "first_name": author.first_name}
            for author in doc.authors
        ]:
            try:
                existing_author = get_user_author(
                    user, submitted_author["first_name"], submitted_author["last_name"]
                )
            except NoResultFound:
...
def delete_orphaned_authors():
    ...
    orphaned_authors_sql = (
        "delete from authors where id not in (select author_id from document_authors)"
    )
...
def edit_item(content, user, source=""):
    ...
    try:
        ...
        if source_id == 3 and link:
            doc = (
                user.documents.filter(Documents.link == link)
...
def remove_to_read(source):
    ...
    (
        current_user.documents.filter(Documents.source_id == source, Documents.read == 0).delete(
