        ...
        for i in range(1, pages+1):
            ...
            for review in root[2]:  # root[2] is *reviews* top-level xml
                ...
                if update_type == 'initial':
                    ...
                    # edit or add it
                    try:
                        doc = current_user.documents.filter(Documents.source_id==2, Documents.native_doc_id==book['native_doc_id']).one()
                        book['id'] = doc.id
                        try:
...
def get_book_details(review, shelf):
    ...
    if review.find('read_at').text is not None:
        ...
        created = datetime.datetime.strptime(review.find('date_added').text, '%a %b %d %H:%M:%S %z %Y')
